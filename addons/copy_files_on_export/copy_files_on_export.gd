...
func _feature_match(
	requested_features: PackedStringArray, limited_features: PackedStringArray
) -> bool:
	...
	if DirAccess.dir_exists_absolute(source_path):
		...
		for sub_path in sub_paths:
			if _ignore_path(source_path.path_join(sub_path)):
				continue
			_write_to_zip(
				zip_packer, source_path.path_join(sub_path), dest_path.path_join(sub_path)
			)
		return
...
func _ignore_path(path: String) -> bool:
	...
	return (
		...
		and (path.ends_with(".import") or path.ends_with(".uid"))
	)
