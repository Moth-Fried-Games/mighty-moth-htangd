...
enum ShowMode { ANY, KEYBOARD_MOUSE, CONTROLLER }  ## Icon will be display on any input method.  ## Icon will be display only when the keyboard/mouse is being used.  ## Icon will be display only when a controller is being used.
...
@export
var force_controller_icon_style: ControllerSettings.Devices = ControllerSettings.Devices.NONE:
...
func _on_label_settings_changed():
	_font = ThemeDB.fallback_font if not _label_settings.font else _label_settings.font
	_text_size = _font.get_string_size(
		"+", HORIZONTAL_ALIGNMENT_LEFT, -1, _label_settings.font_size
	)
	_reload_resource()
...
func _load_texture_path_impl():
	...
	if ControllerIcons.is_node_ready() and _can_be_shown():
		var input_type = (
			ControllerIcons._last_input_type if force_type == ForceType.NONE else force_type - 1
		)
		...
		var target_device = (
			force_device if force_device != ForceDevice.ANY else ControllerIcons._last_controller
		)
		var tex := ControllerIcons.parse_path(
			path, input_type, target_device, force_controller_icon_style
		)
		if tex:
			textures.append(tex)
	_textures = textures
...
func _get_width() -> int:
	if _can_be_shown():
		var ret := _textures.reduce(
			func(accum: int, texture: Texture2D):
				if texture:
...
func _has_alpha() -> bool:
	return _textures.any(func(texture: Texture2D): return texture.has_alpha())
	...
	for i in range(_textures.size()):
		...
		if i != 0:
			...
			var font_position := Vector2(
				position.x, position.y + (get_height() - _text_size.y) / 2.0
			)
...
func _draw_rect_region(
	...
	for i in range(_textures.size()):
		...
		var src_rect_ratio := Vector2(
			tex.get_width() / float(_get_width()), tex.get_height() / float(_get_height())
		)
		var tex_src_rect := Rect2(
			src_rect.position * src_rect_ratio, src_rect.size * src_rect_ratio
		)

		tex.draw_rect_region(
			to_canvas_item, Rect2(position, size), tex_src_rect, modulate, transpose, clip_uv
		)
		position.x += size.x
...
func _draw_text(to_canvas_item: RID, font_position: Vector2, text: String):
	...
	if _label_settings.shadow_color.a > 0:
		_font.draw_string(
			to_canvas_item,
			font_position + _label_settings.shadow_offset,
			text,
			HORIZONTAL_ALIGNMENT_LEFT,
			-1,
			_label_settings.font_size,
			_label_settings.shadow_color
		)
		if _label_settings.shadow_size > 0:
			_font.draw_string_outline(
				to_canvas_item,
				font_position + _label_settings.shadow_offset,
				text,
				HORIZONTAL_ALIGNMENT_LEFT,
	...
	if _label_settings.outline_color.a > 0 and _label_settings.outline_size > 0:
		_font.draw_string_outline(
			...
			-1,
			_label_settings.font_size,
			_label_settings.outline_size,
			_label_settings.outline_color
		)
	_font.draw_string(
		to_canvas_item,
		font_position,
	...
	for i in range(_textures.size()):
		...
		if i != 0:
			...
			var font_position := Vector2i(
				position.x, position.y + (get_height() - region.size.y) / 2
			)
		...
		img.blit_rect(
			texture_raw, Rect2i(0, 0, texture_raw.get_width(), texture_raw.get_height()), position
		)
